...
def enable_vm_autoshutdown(vm_name, resource_group_name, time, timezone_id='UTC'):
    ...
    schedule = az_cli(['resource', 'create',
                       '-g', resource_group_name,
                       '-n', 'shutdown-computevm-{}'.format(vm_name),
...
def disable_vm_autoshutdown(vm_name, resource_group_name):
    ...
    resource_id = '/subscriptions/{}/resourcegroups/{}/providers/microsoft.devtestlab/schedules/shutdown-computevm-{}'.format(
        subscription_id, resource_group_name, vm_name)
    return az_cli(['resource', 'delete',
                   '--ids', resource_id])


def show_vm_autoshutdown(cmd, vm_name, resource_group_name):
    access_token, subscription_id = get_access_token()
    schedules = get_resources('Microsoft.DevTestLab', 'schedules',
                              access_token=access_token,
                              subscription_id=subscription_id,
                              api_version='2016-05-15')

    searchString = '^/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Compute/virtualMachines/{}$'.format(
        subscription_id, resource_group_name, vm_name)
    regexp = re.compile(searchString)

    activeSchedules = [x for x in schedules
                       if x['properties']['taskType'] == 'ComputeVmShutdownTask'
                       and regexp.search(x['properties']['targetResourceId'])]

    return activeSchedules[0] if activeSchedules else None
...
def get_resources(namespace, resource_type, access_token=None, subscription_id=None, api_version=None):
